...
export default function App() {
  ...
  return (
    <div className="min-h-screen bg-[#f0f9ff] text-slate-800 font-sans antialiased flex flex-col selection:bg-teal-500/30 selection:text-teal-950 transition-colors duration-200">
          ...
          <>
            ...
            <div className="space-y-3">
              <div className="flex items-center justify-between px-1">
                <h2 className="text-xs font-bold uppercase text-slate-650 tracking-widest flex items-center gap-2">
              ...
              <div className="bento-card p-5 bg-sky-100 border border-sky-200 text-slate-800 shadow-md relative overflow-hidden rounded-xl">
                ...
                <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                  <div className="space-y-1.5">
                    <div className="flex items-center gap-2">
                      <span className={`text-[9px] ${strategy === 'bull' ? 'bg-rose-50 text-rose-700 border border-rose-200' : 'bg-emerald-50 text-emerald-700 border border-emerald-200'} px-2 py-0.5 rounded font-bold uppercase tracking-wider`}>
                        目前決策摘要
                      </span>
                    </div>
                    <h4 className="text-base font-bold font-display text-slate-850 flex items-center gap-1.5">
                      <span className={`w-2 h-2 rounded-full ${strategy === 'bull' ? 'bg-rose-600 animate-ping' : 'bg-emerald-500 animate-ping'}`}></span>
                      {strategy === "bull" ? "賣權看多價差" : "買權看空價差"} (收 {params.premium} 點)
                    </h4>
                    <p className="text-xs text-slate-650 leading-relaxed max-w-4xl">
                      於履約差 <b className="text-slate-850">{params.strikeDifference} 點</b> 框架下承作，預期年化/單期獲利率為 <b>{formatPercentVal(params.targetReturnRate)}</b>。
                      系統推算每組防震配資規模為 <b className="text-violet-700 font-mono font-bold">{formatTWD(results.capitalCost)}</b>。
                    </p>
                  </div>
                  
                  <div className="flex flex-col items-start sm:items-end justify-center bg-white border border-sky-200 px-5 py-3 rounded-xl min-w-[200px] shadow-inner">
                    <span className="text-[10px] text-slate-500 uppercase tracking-wider font-bold">剩餘保留可用餘額</span>
                    <span className="text-base font-mono font-extrabold text-emerald-600 mt-0.5">
            ...
            <div className="space-y-4">
              <h2 className="text-xs font-bold uppercase text-slate-650 tracking-widest flex items-center gap-2 px-1">
            ...
            <div className="space-y-4 pt-4">
              <div className="flex items-center justify-between border-b border-sky-200 pb-3 p-1">
                <h2 className="text-xs font-bold uppercase text-slate-750 tracking-widest flex items-center gap-2">
      ...
      <footer className="border-t border-sky-200 mt-20 py-10 text-xs bg-sky-100/40 text-slate-500 transition-colors duration-200">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center space-y-3">
          <p className="font-mono tracking-wider text-[10px] uppercase text-slate-500">
            © 2026 選擇權量化策略研究室 // 選擇權評估系統
          </p>
